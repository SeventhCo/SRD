...
namespace Proyecto
{
    class Logica
    {
        ...
        public static byte registrar(string gmail, string dominio, string contra , string nacimiento , string nombre , string apellido , string pais ,string vencimiento)
        {
            ...
            {
                cm.Execute("insert into usuario values ('"+gmail+"','"+dominio+"',md5('"+contra+"'),"+0+",'"+nacimiento+"','"+nombre+"','"+apellido+"','"+pais+"','"+vencimiento+"')", out cantFilas);
                cm.Close();
                return 0;
            }
            else
        ...
        public static byte iniciar(string gmail,string dominio, string contra)
        {
            ...
            {
                cm.Execute("SELECT * FROM usuario WHERE gmail = '" + gmail + "' AND dominio = '" + dominio + "' AND contra = md5('" + contra + "') AND rol > 0", out cantFilas);
                cm.Close();
                if (cantFilas.ToString().Equals("0"))
                {
                    return 1;
        ...
        public static bool addPublicidad(string identificador, string tipo, string pais,Image publicidad)
            {
                ...
                { MessageBox.Show("Esta imagen ya fue ingresada");
                  cm.Close();
                }
                return true;
                }

            ...
            if ((int.TryParse(textBox4.Text, out dia) && dia > 0 && dia < 32 && int.TryParse(textBox6.Text, out mes) && mes > 0 && mes < 13 && int.TryParse(textBox7.Text, out ano) && ano >= 1900))
            {
                ...
                switch (contador)
                {
                    case 1:
                        ...
                        Usuarios usr = new Usuarios(mail[0], mail[1], textBox3.Text,0, fecha, textBox1.Text, textBox5.Text, "", "");
                        Retorno ret = JsonConvert.DeserializeObject<Retorno>(ApiAux.registrar(JsonConvert.SerializeObject(usr)));
                        switch (ret.result)
                        {
                            case 0:

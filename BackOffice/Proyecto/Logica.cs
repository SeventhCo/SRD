...
namespace Proyecto
{
    class Logica
    {
        ...
        public static bool addDepo(string sisPuntaje,string nombreDep, string nombreCorto, bool indiv, int N_titulares, Image logo)
        {
            ...
            if (cantFilas.ToString().Equals("0"))
            {
                cm.Execute("insert into deporte values ('"+ sisPuntaje + "','" + nombreDep + "','" + nombreCorto + "'," + indiv + "," + N_titulares + ",'"+img+"','')", out cantFilas);
                cm.Close();
                return true;
            }
            else
        ...
        public static void addEnfrentamiento(string fecha, List<string> equipos,string torneo,string deporte,string titulo)
        {
            cm.Open("bd7sdco", "root");

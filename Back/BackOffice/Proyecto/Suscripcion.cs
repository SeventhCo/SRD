...
namespace Proyecto
{
    public partial class Suscripcion : Form
    {
        ...
        private void btnSuscribirse_Click(object sender, EventArgs e)
        {
            ...
            dynamic[,] datosUsuario = Logica.consulta("Select rol from usuario WHERE gmail = '" + email[0] + "' AND dominio = '" + email[1] + "'");
            ...
            else
            {
                if (datosUsuario != null)
                {
                    ...
                    {
                        Logica.comando("Update usuario set rol = 1 where gmail = '" + email[0] + "' and dominio = '" + email[1] + "'");

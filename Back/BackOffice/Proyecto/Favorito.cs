...
namespace Proyecto
{
    public partial class Favorito : Form
    {
        ...
        public void cargaEqui()
        {
            ...
            for(int k = 0; k < Logica.consultaMaxIndex("select nombreEqu from favEquipo where gmail = '" + Program.Blank.user.gmail[0] + "' and dominio = '" + Program.Blank.user.gmail[1] + "'");k++)
            {
                for (int i = 0; i < Logica.consultaMaxIndex("Select * from equipo where nombreEqu = '" + Logica.consulta("select nombreEqu from favEquipo where gmail = '" + Program.Blank.user.gmail[0] + "' and dominio = '" + Program.Blank.user.gmail[1] + "'")[0, k] + "'"); i++)
                {
                    dynamic[,] equiInfo = Logica.consulta("Select * from equipo where nombreEqu = '" + Logica.consulta("select nombreEqu from favEquipo where gmail = '" + Program.Blank.user.gmail[0] + "' and dominio = '" + Program.Blank.user.gmail[1] + "'")[0, k] + "'");

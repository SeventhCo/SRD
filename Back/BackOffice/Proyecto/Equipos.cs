            ...
            if(errores == 0)
            {
                for (int i = 0; i < Program.Blank.torn.vinculos.Count; i++)
                {
                    if (Program.Blank.torn.vinculos[i][0].Controls[0].Text == "1")
                    {
                        ...
                        if (Logica.consultaMaxIndex("select * from Torneo where nombreTorn = '" + Program.Blank.torn.torneo + "'") > 0)
                        {
                            ...
                            if (Logica.consultaMaxIndex("select * from fase where nombreFase = '" + Program.Blank.torn.vinculos[i][0].Name + "' and nombreTorn = '" + Program.Blank.torn.torneo + "'")>0)
                            {
